...
public class CreditoServiceImpl implements CreditoService {
	...
	public List<DetalleCronogramaCreditoDto> getSimulacionCronograma(DatosCreditoDto datosCreditoDto) {
		...
		LocalDate[] fechasVencimiento = getFechasVencimiento(LocalDate.parse(datosCreditoDto.getFechaPrimeraCuota()), 
				FrecuenciaCredito.valueOf(datosCreditoDto.getFrecuencia()), datosCreditoDto.getNroCuotas().intValue());
		
		int[] diferenciaEnDias = getDiferenciaEnDias(LocalDate.parse(datosCreditoDto.getFechaDesembolso()), fechasVencimiento);
		
		double ted = getTEDFromTEM(datosCreditoDto.getTem().doubleValue() / 100);
		
		BigDecimal montoCuota = getMontoCuota(datosCreditoDto.getMontoDesembolso(), diferenciaEnDias, ted);
		
		List<DetalleCronogramaCreditoDto> simulacionCronogramaDto = new ArrayList<>();
		
		// 1era Linea es Detalle Desembolso
		DetalleCronogramaCreditoDto firstDetalleCronograma = new DetalleCronogramaCreditoDto(0, datosCreditoDto.getFechaDesembolso(),
				datosCreditoDto.getMontoDesembolso(), new BigDecimal(0), new BigDecimal(0), montoCuota);
		simulacionCronogramaDto.add(firstDetalleCronograma);
		
		for (int i = 0; i < datosCreditoDto.getNroCuotas().intValue(); i++) {
			...
			// ultima cuota
			if (nroCuota == datosCreditoDto.getNroCuotas().intValue() ) {
